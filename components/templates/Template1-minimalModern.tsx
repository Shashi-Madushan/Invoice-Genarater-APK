...
export const Template1: React.FC<Props> = ({ data }) => {
  return (
    <View className="p-4 border border-gray-300 rounded-lg w-[300px] bg-white">
      <View className="flex-row justify-between items-center mb-2.5">
        {data.logoUrl && (
          <Image source={{ uri: data.logoUrl }} className="w-10 h-10" />
        )}
        <Text className="text-xl font-bold">INVOICE</Text>
        <Text className="text-xs">No. 000001</Text>
      </View>

      <Text className="mb-2.5">Date: {data.date}</Text>

      <View className="flex-row justify-between border-b pb-1">
        <Text className="w-16 text-center text-xs">Item</Text>
        <Text className="w-16 text-center text-xs">Qty</Text>
        <Text className="w-16 text-center text-xs">Price</Text>
      ...
      <View className="mt-2.5">
        ...
        <Text className="font-bold">Total: ${data.totals.grandTotal}</Text>
      </View>

      <Text className="mt-1.5 text-xs">Payment method: Cash</Text>
      <Text className="mt-1.5 text-xs">Note: Thank you for choosing us!</Text>
    </View>
  );
};