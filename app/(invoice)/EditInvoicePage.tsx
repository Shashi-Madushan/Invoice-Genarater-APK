...
export default function EditInvoicePage() {
  ...
  const parsedData = invoiceData ? JSON.parse(invoiceData as string) : {};

  // State for form fields (pre-filled)
  const [storeName, setStoreName] = useState(parsedData.storeName || "");
  const [logoUrl, setLogoUrl] = useState(parsedData.logoUrl || "");
  const [date, setDate] = useState(parsedData.date ? new Date(parsedData.date).toISOString().slice(0,10) : "");
  const [shopDetails, setShopDetails] = useState<ShopDetails>(parsedData.shopDetails || { name: "" });
  const [customerDetails, setCustomerDetails] = useState<CustomerDetails>(parsedData.customerDetails || { name: "" });
  const [items, setItems] = useState<InvoiceItem[]>(parsedData.items || [{ name: "", quantity: 1, price: 0 }]);
  const [totalDiscount, setTotalDiscount] = useState(parsedData.totals?.totalDiscount || 0);
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        ...
        <Text className="text-base text-gray-700 mb-4 text-center">
          Template ID: <Text className="font-semibold text-purple-600">{parsedData.templateId}</Text>
        </Text>
